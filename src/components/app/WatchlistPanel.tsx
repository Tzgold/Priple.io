...
import { ScreenerTokenColumn, type ScreenerListCoin } from "@/components/app/ScreenerTokenColumn";
...
export function WatchlistPanel({ workspace }: { workspace: string }) {
  ...
  const onScreener = pathname.startsWith("/app/screener");
  const onWallets = pathname.startsWith("/app/wallets");
  ...
  const activeId = (() => {
    ...
    if (!onScreener || !network || !address) return null;
  ...
  function openCoin(coin: ScreenerListCoin) {
    ...
    const href = `/app/screener?${params.toString()}`;
    if (pathname.startsWith("/app/screener")) router.replace(href);
    else router.push(href);
  }

  function openPair(hit: PairHit) {
    const href = `/app/screener?network=${encodeURIComponent(hit.network)}&address=${encodeURIComponent(hit.address)}`;
    if (pathname.startsWith("/app/screener")) router.replace(href);
    else router.push(href);
  }
      ...
      {onWallets ? (
        ...
        <div className="mt-4 flex min-h-0 flex-1 flex-col overflow-hidden">
          <ScreenerTokenColumn
            trending={trending}
            watchlist={watchlist}
            walletBuys={walletBuys}
            majors={majors}
            topScore={topScore}
            activeId={activeId}
            loading={loading}
            onSelect={openCoin}
            searchSlot={<PairSearchBox onSelect={openPair} />}
          />
        </div>
      )}
    </aside>
